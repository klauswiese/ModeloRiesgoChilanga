# Mapeo de la amenaza
#Chilanga
#https://eenew.ifrc.org/x/nKx5GoA9
#-------------------------------------------------------------------------------
...
asset <- kobo_asset("aKH2gaQRfcSmz45ZnFyV32")
KOBO <- kobo_data(asset) %>%
  dplyr::select("muni","amenaza","otra_amenaza", "A1":"C6")
...
x <- grep("Area", AmenazaMun$Chilanga$Aldea)
AmenazaMun$Chilanga$Aldea[x[1]] <- "Área Urbana - Amenaza"
AmenazaMun$Chilanga$Aldea[x[2]] <- "Área Urbana - Falta de Capacidad de Respuesta"
AmenazaMun$Chilanga$Aldea[x[3]] <- "Área Urbana - Vulnerabilidad"
...
library(sf)
aldeas_sf <- st_read('GPKG/Chilanga2.gpkg') %>%
  st_transform(4326) %>%
...
ExpoAldea <- function(n){
  ...
  data_map2 <- 
    ...
    addLegend("topright", pal = pal, values = ~Valor,
              title = md(paste0("Exposición y Peligro <br>", "Chilanga")),
              opacity = 1)
...
VulnerabilidadAldea <- function(n){
  ...
  data_map2 <- 
    ...
    addProviderTiles(providers$Esri.WorldImagery) %>%
    addLegend("topright", pal = pal, values = ~Valor,
              title = md(paste0("Vulnerabilidad <br>", "Chilanga")),
              opacity = 1)
...
FaltaAldea <- function(n){
  ...
  data_map2 <- 
    ...
    addProviderTiles(providers$Esri.WorldImagery) %>%
    addLegend("topright", pal = pal, values = ~Valor,
              title = md(paste0("Falta de Capacidad de Respuesta <br>", "Chilanga")),
              opacity = 1)
...
RiesgoAldea <- function(n){
  ...
  data_map2 <- 
    ...
    addProviderTiles(providers$Esri.WorldImagery) %>%
    addLegend("topright", pal = pal, values = ~Valor,
              title = md(paste0("Riesgo <br>", "Chilanga")),
              opacity = 1)
